...
func TestDrillAttachPicker(t *testing.T) {
	...
	// The BOARD-origin picker (formOrigin=modeBoard) is a distinct reachable branch:
	...
	t.Run("board-origin cancel restores the board and keeps the ready-ship selection", func(t *testing.T) {
		...
		m = nm.(Model)
		if m.mode != modeForm || m.pendingAttach == nil || m.formOrigin != modeBoard {
			t.Fatalf("board a did not open a board-origin attach picker (mode=%d pending=%v origin=%d)", m.mode, m.pendingAttach, m.formOrigin)
		}
