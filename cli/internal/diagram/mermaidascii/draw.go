...
func wrapTextInColor(text, c, styleType string) string {
	...
	// Vendoring change: upstream's "cli" branch wrapped text in ANSI color via
	// gookit/color. We emit plain text so diagrams stay clean inside terminal
	// viewports and markdown code blocks (glamour) — and to drop that dep.
	return text
}
