...
(function () {
  ...
  function render(figure, mermaidSvg, model, opts) {
    ...
    // refit re-fits the whole diagram to the CURRENT viewport size and redraws —
    // the entry point the expand-to-modal control calls after the .viewport node
    // moves into (or back out of) the near-fullscreen overlay (TEST-012).
    function refit() { fitAll(); draw(); }

    return { viewport: viewport, redraw: draw, resetLayout: resetLayout, refit: refit };
  }
