 ...
 * very-nice-mermaid v0.7.1 (MIT) - https://github.com/ZawadzkiB/very-nice-mermaid
...
var require_normalize = __commonJS({
  "node_modules/@dagrejs/dagre/lib/normalize.js"(exports, module) {
    ...
    function normalizeEdge(g, e) {
      ...
      let name = e.name;
      let edgeLabel4 = g.edge(e);
      let labelRank = edgeLabel4.labelRank;
      ...
      for (i = 0, ++vRank; vRank < wRank; ++i, ++vRank) {
        ...
        attrs = {
          width: 0,
          height: 0,
          edgeLabel: edgeLabel4,
          edgeObj: e,
          rank: vRank
        };
        dummy = util.addDummyNode(g, "edge", attrs, "_d");
        if (vRank === labelRank) {
          attrs.width = edgeLabel4.width;
          attrs.height = edgeLabel4.height;
          attrs.dummy = "edge-label";
          attrs.labelpos = edgeLabel4.labelpos;
        }
        g.setEdge(v, dummy, { weight: edgeLabel4.weight }, name);
        if (i === 0) {
          g.graph().dummyChains.push(dummy);
        }
        v = dummy;
      }
      g.setEdge(v, w, { weight: edgeLabel4.weight }, name);
    }
    function undo(g) {
    ...
    function findSmallestWidthAlignment(g, xss) {
      return Object.values(xss).reduce((currentMinAndXs, xs) => {
        ...
        Object.entries(xs).forEach(([v, x]) => {
          let halfWidth2 = width(g, v) / 2;
          max = Math.max(x + halfWidth2, max);
          min = Math.min(x - halfWidth2, min);
        });
...
function resolveNodeStyle(node, classDefs, theme) {
...
__export(geometry_exports, {
  LABEL_LINE_GAP: () => LABEL_LINE_GAP,
  NODE_AVOID_MARGIN: () => NODE_AVOID_MARGIN,
  NODE_AVOID_PASSES: () => NODE_AVOID_PASSES,
  SUBGRAPH_AVOID_MARGIN: () => SUBGRAPH_AVOID_MARGIN,
  SUBGRAPH_PADDING: () => SUBGRAPH_PADDING,
  SUBGRAPH_TITLE_BAND: () => SUBGRAPH_TITLE_BAND,
  applyEdgeBridges: () => applyEdgeBridges,
  avoidNodes: () => avoidNodes,
  avoidSubgraphs: () => avoidSubgraphs,
  computeAvoidContainers: () => computeAvoidContainers,
  computePerimeterPorts: () => computePerimeterPorts,
  computeSubgraphBoxes: () => computeSubgraphBoxes,
  contentBounds: () => contentBounds,
  detourApproaches: () => detourApproaches,
  labelPoint: () => labelPoint,
  n: () => n,
  pickSides: () => pickSides,
  raySide: () => raySide,
  resolveLabelCollisions: () => resolveLabelCollisions,
  resolveLabelEdgeCollisions: () => resolveLabelEdgeCollisions,
  resolveLabelLineOffsets: () => resolveLabelLineOffsets,
  resolveLabelNodeCollisions: () => resolveLabelNodeCollisions,
  roundedPath: () => roundedPath,
  routeCurved: () => routeCurved,
  routeEdge: () => routeEdge,
  routeElbow: () => routeElbow,
  segmentsCross: () => segmentsCross,
  separateAntiParallelJogs: () => separateAntiParallelJogs,
  separateConvergentJogs: () => separateConvergentJogs,
  separateLanes: () => separateLanes,
  sidePoint: () => sidePoint,
  subgraphBox: () => subgraphBox,
  toPath: () => toPath,
  trimEndpointReentry: () => trimEndpointReentry
});
var PORT_MARGIN = 6;
var PORT_STEP = 30;
...
function trimEndpointReentry(edges, nodeBoxes, style, pinned) {
  ...
  edges.forEach((e, ei) => {
    ...
    const pin = pinned?.[ei];
    const src = pin?.source ? void 0 : boxById.get(e.from);
    const tgt = pin?.target ? void 0 : boxById.get(e.to);
    let did = false;
...
function finishEdges(edges, theme, bridges, nodeBoxes, subgraphs, pinned) {
  ...
  if (nodeBoxes) trimEndpointReentry(edges, nodeBoxes, theme.edgeStyle, pinned);
...
function flowEdgeSemantic(kind, label) {
  const s = (label ?? "").toLowerCase();
...
var NON_FLOWCHART_HEADERS = /* @__PURE__ */ new Set([
  "sequencediagram",
  "classdiagram",
  "classdiagram-v2",
  "statediagram",
...
function explicitNonFlowchartType(dsl) {
...
function ensureSyncRenderable(input, asyncApi) {
  if (typeof input !== "string") return;
  const type = explicitNonFlowchartType(input);
  if (type) {
    throw new Error(
      `very-nice-mermaid: '${type}' is not a flowchart, so it cannot be rendered by the synchronous API. Use the async \`${asyncApi}(dsl)\` (renders sequence/class/state natively and other types via the mermaid.js fallback), or \`mount()\` / the <very-nice-mermaid> element, which route every diagram type.`
    );
  }
}
...
var SEMANTICS2 = ["request", "response", "cache", "async", "exception"];
function defs(theme, sketch) {
  ...
  return `<defs>` + marker("vnm-arrow", t.colors.edge) + SEMANTICS2.map((s) => marker(markerId(s), semanticColor2(s, t))).join("") + shadow + (sketch ? sketchFontDefs() : "") + `</defs>`;
}
...
function renderMessage(m, theme, sketch, index) {
  ...
  const color = semanticColor2(m.semantic, t);
...
function renderLegend(layout2, theme) {
  ...
  for (const sem of layout2.legend) {
    const color = semanticColor2(sem, t);
    parts.push(
    ...
    const label = SEMANTIC_LABEL2[sem];
    parts.push(
...
function renderClassSvg(layout2, theme, background, style = "clean") {
  const b = layout2.model.bounds;
  const t = theme.tokens;
  const sketch = style === "sketch";
  const byId = new Map(layout2.classes.map((c) => [c.id, c]));
  const parts = [];
  parts.push(
    `<svg xmlns="http://www.w3.org/2000/svg" width="${n(b.width)}" height="${n(
      b.height
  ...
  const count = Math.min(layout2.model.edges.length, layout2.relations.length);
  for (let i = 0; i < count; i++) {
    parts.push(renderRelation(layout2.model.edges[i], layout2.relations[i], theme, sketch));
  }
  for (const node of layout2.model.nodes) {
...
function defs2(theme, sketch) {
  const t = theme.tokens;
  const edge = t.colors.edge;
  const fill = t.colors.surface;
  const shadow = t.effects.gradient ? `<filter id="vnm-shadow" x="-30%" y="-30%" width="160%" height="160%"><feDropShadow dx="0" dy="3" stdDeviation="5" flood-color="#000" flood-opacity="0.35"/></filter>` : "";
  return `<defs><marker id="vnm-cls-tri" viewBox="0 0 14 14" refX="13" refY="7" markerWidth="14" markerHeight="14" orient="auto"><path d="M0 0 L14 7 L0 14 z" fill="${fill}" stroke="${edge}" stroke-width="1"/></marker><marker id="vnm-cls-tri-start" viewBox="0 0 14 14" refX="1" refY="7" markerWidth="14" markerHeight="14" orient="auto"><path d="M14 0 L0 7 L14 14 z" fill="${fill}" stroke="${edge}" stroke-width="1"/></marker><marker id="vnm-cls-diamond-solid" viewBox="0 0 20 12" refX="19" refY="6" markerWidth="18" markerHeight="12" orient="auto"><path d="M20 6 L10 0 L0 6 L10 12 z" fill="${edge}" stroke="${edge}" stroke-width="1"/></marker><marker id="vnm-cls-diamond-solid-start" viewBox="0 0 20 12" refX="1" refY="6" markerWidth="18" markerHeight="12" orient="auto"><path d="M0 6 L10 0 L20 6 L10 12 z" fill="${edge}" stroke="${edge}" stroke-width="1"/></marker><marker id="vnm-cls-diamond-hollow" viewBox="0 0 20 12" refX="19" refY="6" markerWidth="18" markerHeight="12" orient="auto"><path d="M20 6 L10 0 L0 6 L10 12 z" fill="${fill}" stroke="${edge}" stroke-width="1"/></marker><marker id="vnm-cls-diamond-hollow-start" viewBox="0 0 20 12" refX="1" refY="6" markerWidth="18" markerHeight="12" orient="auto"><path d="M0 6 L10 0 L20 6 L10 12 z" fill="${fill}" stroke="${edge}" stroke-width="1"/></marker><marker id="vnm-cls-open" viewBox="0 0 12 12" refX="10" refY="6" markerWidth="11" markerHeight="11" orient="auto"><path d="M1 1 L11 6 L1 11" fill="none" stroke="${edge}" stroke-width="1.4"/></marker><marker id="vnm-cls-open-start" viewBox="0 0 12 12" refX="2" refY="6" markerWidth="11" markerHeight="11" orient="auto"><path d="M11 1 L1 6 L11 11" fill="none" stroke="${edge}" stroke-width="1.4"/></marker>` + shadow + (sketch ? sketchFontDefs() : "") + `</defs>`;
}
function renderRelation(edge, rel, theme, sketch) {
  const t = theme.tokens;
  const dashed = rel.type === "realization" || rel.type === "dependency";
  const dash = dashed ? ` stroke-dasharray="6 4"` : "";
  const marker = rel.head === "from" ? ` marker-start="url(#${markerFor(rel.type)}-start)"` : ` marker-end="url(#${markerFor(rel.type)})"`;
  const parts = [];
  if (sketch) {
    const pts = edge.points.map((p) => [p.x, p.y]);
...
function edgeLabel(label, cx, cy, theme) {
  const t = theme.tokens;
  const { w, h } = labelPlateSize(label, theme);
  return `<rect x="${n(cx - w / 2)}" y="${n(cy - h / 2)}" width="${n(w)}" height="${n(
    h
  )}" rx="${t.radii.label}" fill="${t.colors.edgeLabelBg}"/><text x="${n(cx)}" y="${n(cy)}" fill="${t.colors.edgeLabelText}" font-size="${t.font.size - 1}" text-anchor="middle" dominant-baseline="central">${escapeXml(label)}</text>`;
}
function renderCard(node, entity, theme, sketch) {
  const t = theme.tokens;
  ...
  lines.header.forEach((line, i) => {
    ...
    parts.push(
      `<text x="${n(node.x)}" y="${n(baseline)}" fill="${t.colors.text}" font-size="${t.font.size}" font-weight="700" text-anchor="middle" dominant-baseline="central">${escapeXml(line)}</text>`
    );
  });
  const divider = (dy, idx) => sketch ? sketchLineSvg([[x, dy], [x + w, dy]], t.colors.surfaceStroke, "1", node.id + "#d" + idx) : `<line x1="${n(x)}" y1="${n(dy)}" x2="${n(x + w)}" y2="${n(dy)}" stroke="${t.colors.surfaceStroke}" stroke-width="1"/>`;
...
function rowText(line, x, baseline, theme) {
  const t = theme.tokens;
  return `<text x="${n(x)}" y="${n(baseline)}" fill="${t.colors.text}" font-size="${t.font.size}" text-anchor="start" dominant-baseline="central">${escapeXml(line)}</text>`;
}
...
// src/native/state/svg.ts
function renderStateSvg(layout2, theme, background, style = "clean") {
  const b = layout2.model.bounds;
  const t = theme.tokens;
  const sketch = style === "sketch";
  const kinds = new Map(layout2.states.map((s) => [s.id, s.kind]));
  const parts = [];
  parts.push(
    `<svg xmlns="http://www.w3.org/2000/svg" width="${n(b.width)}" height="${n(
      b.height
  ...
  for (const edge of layout2.model.edges) parts.push(renderTransition(edge, theme, sketch));
  for (const node of layout2.model.nodes) {
...
function renderTransition(edge, theme, sketch) {
  const t = theme.tokens;
  let parts;
  if (sketch) {
    const pts = edge.points.map((p) => [p.x, p.y]);
  ...
  if (edge.label && edge.labelPos) {
    parts.push(edgeLabel2(edge.label, edge.labelPos.x, edge.labelPos.y, theme));
  }
  return parts.join("");
}
function edgeLabel2(label, cx, cy, theme) {
  const t = theme.tokens;
  const { w, h } = labelPlateSize(label, theme);
  return `<rect x="${n(cx - w / 2)}" y="${n(cy - h / 2)}" width="${n(w)}" height="${n(
    h
...
function renderState(node, kind, theme, sketch) {
  const t = theme.tokens;
  const cx = node.x;
  const cy = node.y;
  const shadow = t.effects.gradient && !sketch ? ` filter="url(#vnm-shadow)"` : "";
  ...
  if (kind === "end") {
    const r = Math.min(9, node.width / 2);
    return `<circle cx="${n(cx)}" cy="${n(cy)}" r="${n(r)}" fill="none" stroke="${t.colors.text}" stroke-width="1.5"/><circle cx="${n(cx)}" cy="${n(cy)}" r="${n(r - 4)}" fill="${t.colors.text}"/>`;
  }
  const x = cx - node.width / 2;
  const y = cy - node.height / 2;
  const rect = sketch ? sketchRectSvg(x, y, node.width, node.height, t.colors.surface, t.colors.surfaceStroke, "1.5", node.id) : `<rect x="${n(x)}" y="${n(y)}" width="${n(node.width)}" height="${n(
    node.height
  )}" rx="${t.radii.card + 4}" fill="${t.colors.surface}" stroke="${t.colors.surfaceStroke}" stroke-width="1.5"/>`;
  const text = `<text x="${n(cx)}" y="${n(cy)}" fill="${t.colors.text}" font-size="${t.font.size}" font-weight="${t.font.weight}" text-anchor="middle" dominant-baseline="central">${escapeXml(
    node.label
  )}</text>`;
  return `<g${shadow}>${rect}${text}</g>`;
}
...
function renderSvgFromModel(model, theme, background, style = "clean") {
  ...
  for (const sg of model.subgraphs) parts.push(renderSubgraphBox(sg, theme));
  for (const edge of model.edges) parts.push(renderEdge(edge, theme, sketch));
  for (const edge of model.edges) {
    if (edge.label && edge.labelPos) parts.push(edgeLabel3(edge.label, edge.labelPos.x, edge.labelPos.y, theme));
  }
  for (const sg of model.subgraphs) parts.push(renderSubgraphTitle(sg, theme));
  for (const node of model.nodes) parts.push(renderNode(node, model, theme, sketch));
  for (const edge of model.edges) parts.push(renderEdgeArrowCaps(edge, theme, sketch));
  if (legend.length) parts.push(renderEdgeLegend(legend, theme, b.x + 16, b.y + b.height + legendH / 2));
  parts.push("</svg>");
  return parts.join("\n");
}
function defs4(theme, sketch) {
  const t = theme.tokens;
  const a = t.edge.arrowSize;
  const shadow = t.effects.gradient ? `<filter id="vnm-shadow" x="-30%" y="-30%" width="160%" height="160%"><feDropShadow dx="0" dy="3" stdDeviation="5" flood-color="#000" flood-opacity="0.35"/></filter>` : "";
  const font = sketch ? `<style>${sketchFontFaceCss()}</style>` : "";
  const semMarkers = t.effects.semanticEdges ? SEMANTICS.map(
    (s) => `<marker id="${semanticMarkerId(s)}" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="${a}" markerHeight="${a}" orient="auto"><path d="M0 0 L10 5 L0 10 z" fill="${semanticColor(s, t)}"/></marker>`
  ).join("") : "";
  return `<defs><marker id="vnm-arrow" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="${a}" markerHeight="${a}" orient="auto"><path d="M0 0 L10 5 L0 10 z" fill="${t.colors.edge}"/></marker><marker id="vnm-arrow-start" viewBox="0 0 10 10" refX="1" refY="5" markerWidth="${a}" markerHeight="${a}" orient="auto"><path d="M10 0 L0 5 L10 10 z" fill="${t.colors.edge}"/></marker>` + semMarkers + shadow + font + `</defs>`;
}
function usedSemantics(model) {
...
function renderEdgeLegend(legend, theme, x0, y) {
  const t = theme.tokens;
  const fs = t.font.size - 2;
  const swatch = 22;
  let x = x0;
  ...
  for (const sem of legend) {
    ...
    out.push(
      `<line x1="${n(x)}" y1="${n(y)}" x2="${n(x + swatch)}" y2="${n(y)}" stroke="${color}" stroke-width="${t.edge.width}" stroke-linecap="round" marker-end="url(#${semanticMarkerId(sem)})"/>`
    );
...
function edgeColor(edge, t) {
  const sem = t.effects.semanticEdges ? flowEdgeSemantic(edge.kind, edge.label ?? "") : void 0;
  return sem ? { color: semanticColor(sem, t), marker: semanticMarkerId(sem) } : { color: t.colors.edge, marker: "vnm-arrow" };
}
function renderSubgraphBox(sg, theme) {
  const t = theme.tokens;
...
function renderSubgraphTitle(sg, theme) {
  if (!sg.title) return "";
  const t = theme.tokens;
  const x = n(sg.x - sg.width / 2);
  const y = n(sg.y - sg.height / 2);
  ...
  return `<rect x="${n(x + 12 - pad)}" y="${n(y + 18 - fs + 1)}" width="${n(pw)}" height="${fs + 4}" rx="${t.radii.label}" fill="${t.colors.subgraphFill}"/><text x="${n(x + 12)}" y="${n(y + 18)}" fill="${t.colors.subgraphText}" font-size="${fs}" font-weight="600">${esc(
    sg.title
  )}</text>`;
}
function renderEdge(edge, theme, sketch) {
  const t = theme.tokens;
  const width = edge.kind === "thick" ? t.edge.thickWidth : t.edge.width;
  const dash = edge.kind === "dotted" ? ` stroke-dasharray="2 5"` : "";
...
function renderEdgeArrowCaps(edge, theme, sketch) {
  ...
  if (sketch) {
    ...
    const out2 = [];
    if (edge.arrows.end) out2.push(`<path class="vnm-arrow-cap" d="${openArrowhead(arr[m - 1], arr[m - 2], t.edge.arrowSize, key + "@end")}"${base}/>`);
    if (edge.arrows.start) out2.push(`<path class="vnm-arrow-cap" d="${openArrowhead(arr[0], arr[1], t.edge.arrowSize, key + "@start")}"${base}/>`);
    return out2.join("");
  }
  const out = [];
...
function capEnd(prev, tip, arrowSize, color, width, marker = "vnm-arrow") {
...
function capStart(head, next, arrowSize, color, width) {
...
function sketchEdgePath(edge, color, width, dash, arrowSize) {
  const key = edge.from + "->" + edge.to;
  const pts = edge.points.map((p) => [p.x, p.y]);
  const base = ` fill="none" stroke="${color}" stroke-width="${width}" stroke-linejoin="round" stroke-linecap="round"`;
...
function edgeLabel3(label, cx, cy, theme) {
  const t = theme.tokens;
  const lines = label.split("\n");
  const maxChars = lines.reduce((m, l) => Math.max(m, l.length), 0);
  const w = maxChars * t.font.size * 0.6 + 6;
  ...
  lines.forEach((line, i) => {
    parts.push(
      `<text x="${n(cx)}" y="${n(startY + i * t.font.lineHeight)}" fill="${t.colors.edgeLabelText}" font-size="${t.font.size - 1}" text-anchor="middle" dominant-baseline="central">${esc(line)}</text>`
    );
  });
  return parts.join("");
}
function renderNode(node, model, theme, sketch) {
  const t = theme.tokens;
  ...
  const shape = sketch ? sketchShape(node, escAttr(s.fill), escAttr(s.stroke), strokeWidth, dash) : nodeShape(node, escAttr(s.fill), escAttr(s.stroke), strokeWidth, dash);
...
function sketchShape(node, fill, stroke, strokeWidth, dash) {
  const x = node.x - node.width / 2;
  const y = node.y - node.height / 2;
  ...
  const parts = [`<path d="${rs.fill}" fill="${fill}" stroke="none"/>`];
...
function nodeShape(node, fill, stroke, strokeWidth, dash) {
  const x = node.x - node.width / 2;
  const y = node.y - node.height / 2;
  const w = node.width;
  const h = node.height;
  ...
  const rect = (rx) => `<rect x="${n(x)}" y="${n(y)}" width="${n(w)}" height="${n(h)}" rx="${rx}" ${common}/>`;
  switch (node.shape) {
    case "rounded":
      return rect(14);
    case "stadium":
    ...
    case "subroutine": {
      const inset = 6;
      return rect(4) + `<line x1="${n(x + inset)}" y1="${n(y)}" x2="${n(x + inset)}" y2="${n(
        y + h
      )}" stroke="${stroke}" stroke-width="${strokeWidth}"/><line x1="${n(x + w - inset)}" y1="${n(y)}" x2="${n(x + w - inset)}" y2="${n(
        y + h
    ...
    case "cylinder": {
      ...
      const top = y + ry;
      const bottom = y + h - ry;
      const d = `M ${n(x)} ${n(top)} C ${n(x)} ${n(top - ry * 1.3)} ${n(x + w)} ${n(top - ry * 1.3)} ${n(x + w)} ${n(top)} L ${n(x + w)} ${n(bottom)} C ${n(x + w)} ${n(bottom + ry * 1.3)} ${n(x)} ${n(bottom + ry * 1.3)} ${n(x)} ${n(bottom)} Z`;
...
function polygon(pts, common) {
  const points = pts.map(([px, py]) => `${n(px)},${n(py)}`).join(" ");
  return `<polygon points="${points}" ${common}/>`;
}
function nodeText(node, color, theme) {
  const t = theme.tokens;
  const lines = node.label.length ? node.label.split("\n") : [""];
  const startY = node.y - (lines.length - 1) * t.font.lineHeight / 2;
  return lines.map(
    (line, i) => `<text x="${n(node.x)}" y="${n(startY + i * t.font.lineHeight)}" fill="${color}" font-size="${t.font.size}" font-weight="${t.font.weight}" text-anchor="middle" dominant-baseline="central">${esc(line)}</text>`
  ).join("");
}
function esc(s) {
  return s.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
...
// src/native/sequence/ascii.ts
var G = {
  h: "\u2500",
  hd: "\u254C",
  // dashed horizontal (dashed message)
  v: "\u2502",
  tl: "\u250C",
  tr: "\u2510",
  bl: "\u2514",
...
var Grid = class {
  constructor(cols, height) {
    this.cols = cols;
    this.height = height;
    for (let r = 0; r < height; r++) this.rows.push(new Array(cols).fill(" "));
  }
  cols;
  height;
  rows = [];
...
function renderSequenceAscii(layout2) {
  const parts = layout2.participants;
  if (parts.length === 0) return "";
  const half = parts.map(halfWidth);
  const order = /* @__PURE__ */ new Map();
  parts.forEach((p, i) => order.set(p.id, i));
  const adjLabel = new Array(Math.max(0, parts.length - 1)).fill(0);
  for (const m of layout2.messages) {
    if (m.self) continue;
    const a = order.get(m.from);
    const b = order.get(m.to);
    if (a === void 0 || b === void 0 || Math.abs(a - b) !== 1) continue;
  ...
  const rightPad = Math.max(...layout2.messages.filter((m) => m.self).map(() => 12), 2);
  const cols = centers[centers.length - 1] + half[half.length - 1] + rightPad;
  const HEADER = 3;
  const perMsg = 2;
  const topBoxRow = 0;
  const firstMsgRow = HEADER + 1;
  const msgRows = layout2.messages.length * perMsg;
  ...
  layout2.messages.forEach((m, idx) => {
...
function renderSequenceMarkdown(layout2) {
  return "```\n" + renderSequenceAscii(layout2) + "\n```\n";
}
function drawBox(grid, center2, half, topRow, label) {
  const x1 = center2 - half;
  const x2 = center2 + half;
...
var CORNERS = /* @__PURE__ */ new Set([G2.tl, G2.tr, G2.bl, G2.br]);
function cornerGlyph(vert, horiz) {
  if (vert === "up") return horiz === "right" ? G2.bl : G2.br;
  return horiz === "right" ? G2.tl : G2.tr;
}
var Grid2 = class {
  constructor(cols, height) {
    this.cols = cols;
  ...
  line(x, y, ch) {
    ...
    if (cur === G2.h && ch === G2.v || cur === G2.v && ch === G2.h) {
      this.set(x, y, G2.cross);
      return;
    }
    this.set(x, y, ch);
  }
  ...
  corner(x, y, ch) {
    ...
    if (cur === G2.h || cur === G2.v || cur === G2.cross || CORNERS.has(cur)) {
      this.set(x, y, G2.cross);
      return;
    }
    this.set(x, y, ch);
...
function renderAscii(input, opts = {}) {
...
function draw(model, opts) {
  ...
  const gx = (x) => Math.round((x - b.x) * scaleX);
  const gy = (y) => Math.round((y - b.y) * scaleY);
  const cols = gx(b.x + b.width) + 2;
  const rows = gy(b.y + b.height) + 2;
  const grid = new Grid2(Math.max(cols, 4), Math.max(rows, 4));
  ...
  for (const node of model.nodes) {
    ...
    drawBox2(grid, box, node.label.split("\n")[0] ?? node.id);
  }
  return grid.toString();
}
function boxCells(node, gx, gy) {
...
function drawBox2(grid, box, label) {
  const { x1, y1, x2, y2 } = box;
  grid.set(x1, y1, G2.tl);
  grid.set(x2, y1, G2.tr);
  grid.set(x1, y2, G2.bl);
  grid.set(x2, y2, G2.br);
  for (let x = x1 + 1; x < x2; x++) {
    grid.set(x, y1, G2.h);
    grid.set(x, y2, G2.h);
  }
...
function connect(grid, from, to, arrowEnd, arrowStart) {
  const a = center(from);
  const c = center(to);
  const dx = c.x - a.x;
  const dy = c.y - a.y;
  if (Math.abs(dy) >= Math.abs(dx)) {
    ...
    if (arrowEnd) grid.set(endX, c.y, dx >= 0 ? G2.arrowRight : G2.arrowLeft);
    if (arrowStart) grid.set(startX, a.y, dx >= 0 ? G2.arrowLeft : G2.arrowRight);
  }
}
...
function renderSequenceHtml(layout2, theme, opts = {}) {
  const payload = buildSequencePayload(layout2, theme, opts);
...
var LS2 = String.fromCharCode(8232);
var PS2 = String.fromCharCode(8233);
function embedJson2(value) {
  return JSON.stringify(value).split("<").join("\\u003c").split(LS2).join("\\u2028").split(PS2).join("\\u2029");
}
function renderHtml(input, opts = {}) {
  if (isSequenceLayout(input)) {
  ...
  const json = embedJson2(payload);
  const title = escHtml2(opts.title ?? "Diagram");
  const bg = theme.tokens.colors.background;
  const bodyFont = opts.style === "sketch" ? SKETCH_FONT_FAMILY : theme.tokens.font.family;
  return `<!doctype html>
<html lang="en">
...
// src/export/png.ts
var INSTALL_HINT = "PNG output requires the optional dependency '@resvg/resvg-js'. Install it with:\n\n  npm install @resvg/resvg-js\n";
...
async function renderPng(input, opts = {}) {
  const fonts = opts.style === "sketch" ? sketchFontRegistration() : void 0;
  return renderPngFromSvg(renderSvg(input, opts), opts.scale, fonts);
}
function sketchFontBytes() {
  return Uint8Array.from(Buffer.from(SKETCH_FONT_BASE64, "base64"));
}
function sketchFontRegistration() {
  return { buffers: [sketchFontBytes()], defaultFontFamily: SKETCH_FONT_NAME };
}
async function renderPngFromSvg(svg, scale, fonts) {
  const { Resvg } = await loadResvg();
  const z = scale && scale > 0 ? scale : 1;
  const font = fonts ? { fontBuffers: fonts.buffers, loadSystemFonts: true, defaultFontFamily: fonts.defaultFontFamily ?? "" } : void 0;
...
function formatRenderDiagnostic(d) {
  const tier = d.tier ?? "-";
...
function buildClassPayload(layout2, theme, opts = {}) {
  return buildPayload(layout2.model, theme, { ...opts, arrowCaps: false });
}
...
function readNodes(doc) {
  ...
  for (const g of Array.from(doc.querySelectorAll("g.node"))) {
    ...
    const center2 = parseTranslate(g.getAttribute("transform")) ?? { x: 0, y: 0 };
    nodes.push({ id: name, label: kind === "normal" ? label : "", kind, center: center2 });
  }
  return nodes;
}
...
function buildStatePayload(layout2, theme, opts = {}) {
  return buildPayload(layout2.model, theme, { ...opts, arrowCaps: false });
}
...
// src/render/route.ts
function warnSketchFallback(detected) {
...
function asciiUnavailable(type) {
  return new Error(
    `very-nice-mermaid: ASCII/Markdown output is unavailable for '${type}' \u2014 only flowchart and sequence diagrams render as ASCII (FR4). Use renderSvgAsync/renderHtmlAsync.`
  );
}
async function renderSvgAsync(dsl, opts = {}) {
  const c = await classify(dsl);
  const theme = resolveTheme(opts.theme);
  switch (c.renderer) {
    case "flowchart":
      return renderSvg(dsl, opts);
    // sync own-parser fast path — no mermaid
    case "sequence":
...
async function renderHtmlAsync(dsl, opts = {}) {
  const c = await classify(dsl);
  const theme = resolveTheme(opts.theme);
  switch (c.renderer) {
    case "flowchart":
...
async function renderAsciiAsync(dsl, opts = {}) {
  const c = await classify(dsl);
  const theme = resolveTheme(opts.theme);
...
async function renderMarkdownAsync(dsl, opts = {}) {
  const c = await classify(dsl);
  const theme = resolveTheme(opts.theme);
...
function wrapFallbackHtml(svg, title, theme) {
...
var DeferredHandle = class {
  ...
  _fail(err) {
    if (typeof console !== "undefined") console.error("very-nice-mermaid:", err?.message ?? err);
  }
  destroy() {
    this.destroyed = true;
    this.real?.destroy();
...
window.vnm = {
  ...
  UNMAPPED_TOKEN_GROUPS: UNMAPPED_TOKEN_GROUPS,
  buildClassPayload: buildClassPayload,
  buildSequencePayload: buildSequencePayload,
  buildStatePayload: buildStatePayload,
  classCardLines: classCardLines,
  classify: classify,
  defineTheme: defineTheme,
  deserializeModel: deserializeModel,
  formatRenderDiagnostic: formatRenderDiagnostic,
  geometry: geometry_exports,
  isClassLayout: isClassLayout,
  isPositionedModel: isPositionedModel,
  isSequenceLayout: isSequenceLayout,
  isStateLayout: isStateLayout,
  layout: layout,
  ...
  mount: mount,
  mountAsync: mountAsync,
  mountClass: mountClass,
  mountSequence: mountSequence,
  mountState: mountState,
  parse: parse,
  readClassModel: readClassModel,
  readSequenceModel: readSequenceModel,
  readStateModel: readStateModel,
  renderAscii: renderAscii,
  renderAsciiAsync: renderAsciiAsync,
  renderClassSvg: renderClassSvg,
  renderFallbackSvg: renderFallbackSvg,
  renderHtml: renderHtml,
  renderHtmlAsync: renderHtmlAsync,
  renderMarkdown: renderMarkdown,
  renderMarkdownAsync: renderMarkdownAsync,
  renderPng: renderPng,
  renderPngFromSvg: renderPngFromSvg,
  renderSequenceAscii: renderSequenceAscii,
  renderSequenceHtml: renderSequenceHtml,
  renderSequenceMarkdown: renderSequenceMarkdown,
  renderSequenceSvg: renderSequenceSvg,
  renderStateSvg: renderStateSvg,
  renderSvg: renderSvg,
  renderSvgAsync: renderSvgAsync,
  renderSvgFromModel: renderSvgFromModel,
  resolveTheme: resolveTheme,
  serializeModel: serializeModel,
  themeCssVars: themeCssVars,
  themes: themes,
  toMermaidTheme: toMermaidTheme
};
