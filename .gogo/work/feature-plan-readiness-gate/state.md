# State — feature `plan-readiness-gate`
...
<!-- Files in this folder (.gogo/work/feature-<slug>/):
  ...
  - (optional) the `correlation:` line — the plan id(s) this work item belongs to (a LIST; stamped when spawned via `/gogo:plan --correlation plan-XXXX`; absent = the item belongs to no cross-source plan)
  - report/        — the as-built bundle (written at phase ⑤): report.md + the UML set (.mmd) + report/before/ (the plan-time "before" set, copied in for before/after compare) + layouts.json + result.json
  - charts/        — mermaid diagrams (.mmd) + charts/before/ (the plan-time as-is baseline) + manifest.json + layouts.json
