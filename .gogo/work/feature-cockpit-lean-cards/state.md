# State — feature `cockpit-lean-cards`
...
- **feature:** Lean cockpit cards — drop the needs-you strip + phase dots; keep status pill; add live agent chip (presentation-only, cli/internal/tui/)
...
- **branch:** main
- **iterations:** plan=0 · implement=1 · review=1 · test=1 · uat=1   <!-- add · uat=N once a UAT round loops back to planning -->
- **resume:** none — shipped to .gogo/changelog/2026-07-14-cockpit-lean-cards/   <!-- <phase to re-enter> — <next action> | none -->
- **open-decision:** none    <!-- D1 (agent chip only when live) + D2 (remove 1..9 number-key) pre-confirmed with the user; not open -->
