# UAT — feature `cockpit-lean-cards`
...
Verified the leaner cockpit board: `⏸ NEEDS YOU` strip and per-card `①②③④⑤` phase dots removed, the `1..9` gate number-key retired, status pill kept, and the live-only `● <agent>` chip shows on non-gate cards. Review CLEAN / test GREEN.
