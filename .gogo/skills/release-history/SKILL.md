...
description: >-
  Use when a gogo change touches the mechanism, rationale, or history of a
  SPECIFIC past release — contracts/validate gates, workspace/changelog/viewer,
  the CLI cockpit and persistent sessions, projects/sources/plans, gate skips,
  events telemetry, session binding, the sessions panel — and you need the full
  per-release note (what shipped, why, the defects it fixed). The always-read
  project-knowledge.md keeps only a one-line index of these mechanisms.
---

# release-history — the per-release gogo overrides notes (0.2.0 → 0.33.0)

Lifted from `knowledge/project-knowledge.md › gogo overrides` by /gogo:skills on 2026-08-02.

**Append future release notes HERE, not to project-knowledge.md** — the parent
keeps only its one-line mechanism index (add your new title there).

## When this applies
A plan/review/report task needs the detail or rationale of a specific shipped
mechanism below. For "what is this project / how is it built", the parent
project-knowledge.md alone is enough.

## The notes (chronological, verbatim)
...
- **Go-fast mode — the token-lean pipeline (since 0.34.0):** `/gogo:go <slug> --fast`
  loads the `gogo-fast` skill: ②→⑤ as ONE warm build+verify context (implement +
  self-review + self-test, no per-round artifacts, an OBJECTIVE green bar), ONE fresh
  `gogo-reviewer` pass (2-round bound with a delta re-review), an in-context final
  suite run, and a short report — same events vocabulary/order, same statuses, still
  landing at `awaiting-uat` with `report/report.md` present. Per-source opt-in:
  `Source.fastMode` in the CLI config appends `--fast` on every go-launch (headless,
  board `g`, auto-pickup) with a visible note; the additive `- **mode:** fast`
  state.md line renders a display-only `⚡fast` chip. `/gogo:done` records findings
  shipped open as `accepted-by-user` (issues-list schema gained `accepted`).
- **Border-only card selection (0.35.0):** focusing/selecting a cockpit card no longer
  repaints its content — one styled body for every state; focus = a double-line border
  in the column accent (`cardFocusedSelected` keeps the select accent), the `┃` gate
  stripe COMPOSES over the chosen border via `GetBorderStyle()`, plans-tab card
  mirrored, changelog/config rows keep their colours (cursor + bright slug), all five
  `plain bool` params retired, `gateBorder`/`changelogFocusStyle` deleted. First
  feature shipped through gogo-fast (3 agent spawns, 17 artifact files / 65KB vs
  ~800KB for a comparable full run). Version **0.35.0**.
