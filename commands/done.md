---
description: Ship a report-complete feature — copy its report bundle into the append-only .gogo/changelog/<date>-<slug>/, build + print the interactive viewer link, and set state.md to a terminal shipped status. Validates inputs.
...
2. **Work** — derive the entry date (the report's `completed:` field, else a date
   ...
   bundle — `report.md` + the `.mmd` UML set + the `before/` set + `diagrams.html`
   (+ `manifest.json`) — into `.gogo/changelog/<date>-<slug>/`. Append-only and
   idempotent: re-running overwrites the same dated dir.
3. **Finish** — set `state.md` to `status: shipped`, `resume: none`; **build the
   interactive viewer page** for the entry (reusing the `gogo-view` build,
   best-effort) and **print its `file://` link** (with the static `diagrams.html`
   path as a fallback — never fail `/gogo:done` over the link); confirm the archived
   path.
